<template>
  <div class="h-full w-full flex">
    ...
    <div class="h-full flex flex-col bg-white rounded-xl transition-all duration-200"
      ...
      <div v-if="pinnedChats.length > 0" class="px-3 pb-1 border-b border-gray-100">
        ...
        <div class="flex overflow-x-auto no-scrollbar gap-1">
          <div v-for="chat in pinnedChats" :key="chat.id"
            ...
            }" @click="selectChat(chat)" @contextmenu.prevent="openContextMenu($event, chat)">
            <n-badge :value="chat.unreadCount" color="red" size="small">
              <n-avatar :size="36" round :src="chat.avatar" class="border border-gray-200 shrink-0" />
            </n-badge>
            <div>
              <span class="text-[10px] font-normal text-gray-500">{{ chat.name }}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <n-virtual-list :items="chatlist" :item-size="68" class="h-full">
          <template #default="{ item, index }">
            <div :key="item.id"
              ...
              }" @click="selectChat(item)" @contextmenu.prevent="openContextMenu($event, item)">
              <div class="flex items-center gap-3">
                <!-- 头像和状态 -->
                <div>
                  <n-badge :value="item.unreadCount" :offset="[-3, 0]" color="red" size="small">
